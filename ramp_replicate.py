...
6/15
    - possible to emulate ramp using linspace segments that evenly space the 
        fabry perot peak (probably a bad idea)
        - use polynomial scale to convert ramp voltage to frequency
        - use residual to find best fit
    - best practice would be to record both the ramp (to find max and min) and
        the associated square wave (to find frequency)
    - gain more precise dither control to account in measurement? or just use 
        averaging <- depends on drift issues
    - for pass through use analog switches that are digitally actuated by the 
        computer
        - need to look at on resistance, switching time, leakage to make sure 
            that such a switch would not affect measured values
...
class Ramp_Replicate():
    ...
    def __init__(self, in_channel = 'ai0', out_channel = 'ao0', ctr_channel = 'ctr0', freq = 100000):
        ...
        '''
        Creates ramp replication object, by default the ramp signal is routed
        through the AO channel
...
## need to add parameters to store PID tuning data
        ...
        self.read_task.timing.samp_clk_rate(100000)
...
# main method runs when file is run
